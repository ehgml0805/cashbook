...
<body class="login-page">
	...
	<form id="updateCashForm"
		action="<%=request.getContextPath()%>/cash/updateCashAction.jsp" method="post">
		...
		<input type="hidden" name="cashNo" value="<%=cashNo%>">
			...
			<div>
				<input type="text" name="cashDate" 
					value="<%=cash.getCashDate() %>" readonly="readonly" class="form-control form-control">
			</div>
			<div style="font-weight: bold;">가격</div>
			<div>
				<input type="text" name="cashPrice" id="cashPrice"
					value="<%=cash.getCashPrice()%>" class="form-control form-control">
			</div>
			<div style="font-weight: bold;">메모</div>
			<div>
				<textarea  class="form-control  form-control-alternative" cols="30" rows="10" name="cashMemo" id="cashMemo"><%=cash.getCashMemo() %></textarea>
			</div>
		<div class="text-center">
			<button type="button" class="btn btn-primary mt-4" id="updateCashBt">수정하기</button>
		</div>
	</form>
	</div>
	</div>
		...
		<script>
		let updateCashBt=document.querySelector('#updateCashBt');
		updateCashBt.addEventListener('click', function(e) {
			console.log('updateCashBt click');
